...
# Selections: objects
from wprime_plus_b.selections.wplusjets.bjet_selection import select_good_bjets
from wprime_plus_b.selections.wplusjets.electron_selection import select_good_electrons
from wprime_plus_b.selections.wplusjets.fatjet_selection import select_good_fatjets
from wprime_plus_b.selections.wplusjets.jet_selection import select_good_jets
from wprime_plus_b.selections.wplusjets.muon_selection import select_good_muons
from wprime_plus_b.selections.wplusjets.tau_selection import select_good_taus
from wprime_plus_b.selections.wplusjets.wjet_selection import select_good_wjets
from wprime_plus_b.selections.wplusjets.mt_selection import select_good_mt
    ...
    def process(self, events):
        ...
        for syst_var in syst_variations:
            ...
            if self.lepton_flavor != "tau":
                lepton_id_config = {
                    "ele": wjets_electron_selection[self.channel][self.lepton_flavor]["electron_id_wp"],
                    "mu": wjets_muon_selection[self.channel][self.lepton_flavor]["muon_id_wp"]
                } 
            ...
            # Cross_cleaning:
            cc = wjets_cross_cleaning_selection[self.channel][self.lepton_flavor]["DR"]


            # select good electrons
            good_electrons = select_good_electrons(
                events=events,
                electron_pt_threshold=wjets_electron_selection[self.channel][
                    self.lepton_flavor
                ]["electron_pt_threshold"],
                electron_eta_threshold = wjets_electron_selection[self.channel][
                    self.lepton_flavor
                ]["electron_eta_threshold"],
                electron_id_wp=wjets_electron_selection[self.channel][
                    self.lepton_flavor
                ]["electron_id_wp"],
                electron_iso_wp=wjets_electron_selection[self.channel][
                    self.lepton_flavor
                ]["electron_iso_wp"],
            )
            ...
            good_muons_masks = select_good_muons(
                events=events,
                muon_pt_threshold=wjets_muon_selection[self.channel][
                    self.lepton_flavor
                ]["muon_pt_threshold"],
                muon_eta_threshold = wjets_muon_selection[self.channel][
                    self.lepton_flavor
                ]["muon_eta_threshold"],
                muon_id_wp= wjets_muon_selection[self.channel][
                    self.lepton_flavor
                ]["muon_id_wp"],
                muon_iso_wp=wjets_muon_selection[self.channel][
                    self.lepton_flavor
                ]["muon_iso_wp"],
            )
            ...
            good_taus_masks  = select_good_taus(
                events=events,
                tau_pt_threshold=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_pt_threshold"],
                tau_eta_threshold=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_eta_threshold"],
                tau_dz_threshold=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_dz_threshold"],
                tau_vs_jet=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_vs_jet"],
                tau_vs_ele=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_vs_ele"],
                tau_vs_mu=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["tau_vs_mu"],
                prong=wjets_tau_selection[self.channel][
                    self.lepton_flavor
                ]["prongs"],
                is_mc=self.is_mc,
            )
            ...
            good_bjets_masks  = select_good_bjets(
                jets=jets_veto,
                year=self.year,
                btag_working_point=wjets_bjet_selection[self.channel][
                    self.lepton_flavor
                ]["btag_working_point"],
                jet_pt_threshold=wjets_bjet_selection[self.channel][
                    self.lepton_flavor
                ]["bjet_pt_threshold"],
                jet_eta_threshold = wjets_bjet_selection[self.channel][
                    self.lepton_flavor
                ]["bjet_eta_threshold"],
                jet_id_wp=wjets_bjet_selection[self.channel][
                    self.lepton_flavor
                ]["bjet_id_wp"],
                jet_pileup_id=wjets_bjet_selection[self.channel][
                    self.lepton_flavor
                ]["bjet_pileup_id"],
                is_mc=self.is_mc,
            )
            ...
            good_jets_masks  = select_good_jets(
                jets=jets_veto,
                year=self.year,
                btag_working_point=wjets_jet_selection[self.channel][
                    self.lepton_flavor
                ]["fail_btag_working_point"],
                jet_pt_threshold=wjets_jet_selection[self.channel][
                    self.lepton_flavor
                ]["jet_pt_threshold"],
                jet_eta_threshold = wjets_jet_selection[self.channel][
                    self.lepton_flavor
                ]["jet_eta_threshold"],
                jet_id_wp=wjets_jet_selection[self.channel][
                    self.lepton_flavor
                ]["jet_id_wp"],
                jet_pileup_id=wjets_jet_selection[self.channel][
                    self.lepton_flavor
                ]["jet_pileup_id"],
                is_mc=self.is_mc,
            )
            ...
            good_fatjets_masks  = select_good_fatjets(
                ...
                year = self.year,
                fatjet_pt_threshold = wjets_fatjet_selection[self.channel][
                    self.lepton_flavor
                ]["fatjet_pt_threshold"],
                fatjet_eta_threshold = wjets_fatjet_selection[self.channel][
                    self.lepton_flavor
                ]["fatjet_eta_threshold"],
                TvsQCD = wjets_fatjet_selection[self.channel][
                    self.lepton_flavor
                ]["TvsQCD"],
                is_mc = self.is_mc,
            )
            ...
            good_wjets_masks  = select_good_wjets(
                wjets = events.FatJet,
                year = self.year,
                w_pt_threshold = wjets_wjet_selection[self.channel][
                    self.lepton_flavor
                ]["wjet_pt_threshold"],
                w_eta_threshold = wjets_wjet_selection[self.channel][
                    self.lepton_flavor
                ]["wjet_eta_threshold"],
                WvsQCD = wjets_wjet_selection[self.channel][
                    self.lepton_flavor
                ]["WvsQCD"],
                is_mc = self.is_mc,
            )
            ...
            if self.is_mc:
                ...
                btag_corrector = BTagCorrector(
                    ...
                    weights=weights_container,
                    sf_type="comb",
                    worging_point=wjets_bjet_selection[self.channel][self.lepton_flavor]["btag_working_point"],
                    tagger="deepJet",
                    year=self.year,
                    full_run=False,
                    variation=syst_var,
                ...
                electron_corrector.add_id_weight(
                    id_working_point=wjets_electron_selection[self.channel][self.lepton_flavor]["electron_id_wp"]
                )
                ...
                # muon corrector
                if (
                    wjets_muon_selection[self.channel][self.lepton_flavor]["muon_id_wp"]
                    == "highpt"
                ):
                ...
                muon_corrector = mu_corrector(
                    ...
                    year=self.year,
                    variation=syst_var,
                    id_wp=wjets_muon_selection[self.channel][self.lepton_flavor]["muon_id_wp"],
                    iso_wp=wjets_muon_selection[self.channel][self.lepton_flavor]["muon_iso_wp"],
                )
                ...
                tau_corrector = TauCorrector(
                    ...
                    weights=weights_container,
                    year=self.year,
                    tau_vs_jet=wjets_tau_selection[self.channel][self.lepton_flavor]["tau_vs_jet"],
                    tau_vs_ele=wjets_tau_selection[self.channel][self.lepton_flavor]["tau_vs_ele"],
                    tau_vs_mu=wjets_tau_selection[self.channel][self.lepton_flavor]["tau_vs_mu"],
                    variation=syst_var,
                )
                ...
                add_QCD_vs_Top_weight(
                        ...
                        year=self.year,
                        year_mod="",
                        working_point_fatjet = wjets_fatjet_selection[self.channel][self.lepton_flavor]["TvsQCD"],
                        variation=syst_var
                )
                ...
                add_QCD_vs_W_weight(
                        ...
                        year=self.year,
                        year_mod="",
                        working_point_wjet = wjets_wjet_selection[self.channel][self.lepton_flavor]["WvsQCD"],
                        variation=syst_var
                )
            ...
            met_threshold =  wjets_met_selection[self.channel][self.lepton_flavor]["met_threshold"]
            ...
            # -------- Trigger: OR ----------#
            reference_trigger =  wjets_trigger_selection[self.channel][self.lepton_flavor]["trigger"]
            if self.lepton_flavor == "mu":
                mu_id = wjets_muon_selection[self.lepton_flavor]["muon_id_wp"]
                with importlib.resources.path(
                    "wprime_plus_b.data", "triggers.json"
                ) as path:
                    with open(path, "r") as handle:
                ...
                ele_id = wjets_electron_selection[self.channel][self.lepton_flavor]["electron_id_wp"]
                with importlib.resources.path(
                    "wprime_plus_b.data", "triggers.json"
                ) as path:
                    with open(path, "r") as handle:
            ...
            # mt cut
            # -------------------------- 
            mt_cut = wjets_mt_selection[self.channel][self.lepton_flavor]["mt_threshold"]
            mt_invert = wjets_mt_selection[self.channel][self.lepton_flavor]["invert"]
            ...
            if nevents_after == 0:
                ...
                cases = []
                cases = [f"case_{i}" for i in range(1, 14) if wjets_cases_selection[self.channel][self.lepton_flavor].get(f"case_{i}", False)]
                ...
                if nevents_top_tagger > 0:
                    ...
                    if self.output_type == "hist":
                        ...
                        array_dict = {}
                        self.add_feature(
                            "weights", weights_container.weight()[region_selection][final_mask] #pre_weights[mask_top]
                        )
                        if self.is_mc == True:
                ...
                if self.is_mc:
                    ...
                    for variation_name, selection in region_selection_map.items():
                        ...
                        if nevents_after_tmp == 0:
                            ...
                            # Top tagger cases
                            cases_tmp = [f"case_{i}" for i in range(1, 14) if wjets_cases_selection[self.channel][self.lepton_flavor].get(f"case_{i}", False)]
