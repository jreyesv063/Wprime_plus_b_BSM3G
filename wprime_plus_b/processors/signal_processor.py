...
class SignalProccessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        # select good light jets: Fail btag
        good_lightjets_masks = select_good_lightjets(
            events=events,
            year=self.year,
            btag_working_point_fail = self.criteria["lightjet"][self.lepton_flavor]["btag_wp_fail"],
            lightjet_pt_threshold = self.criteria["lightjet"][self.lepton_flavor]["pt"],
            lightjet_eta_threshold = self.criteria["lightjet"][self.lepton_flavor]["eta"],
            lightjet_id_wp = self.criteria["lightjet"][self.lepton_flavor]["jet_id"],
            lightjet_pileup_id = self.criteria["lightjet"][self.lepton_flavor]["pileup_id"],
            syst_var=self.syst
        )
        good_lightjets = good_lightjets_masks["nominal"]
        lightjets = events.Jet[good_lightjets]


        # select good topjets: cc = 0.8
        good_topjets_masks = select_good_topjets(
            events=events,
            year=self.year,
            topjet_pt_threshold=self.criteria["topjet"][self.lepton_flavor]["pt"],
            topjet_eta_threshold=self.criteria["topjet"][self.lepton_flavor]["eta"],
            TvsQCD=self.criteria["topjet"][self.lepton_flavor]["particleNet_Top_Nominal"],
            syst_var=self.syst
        )
        good_topjets = good_topjets_masks["nominal"]            
        topjets = events.FatJet[good_topjets]


        # select good W jets: cc = 0.8
        good_wjets_masks = select_good_wjets(
            events=events,
            year = self.year,
            wjet_pt_threshold = self.criteria["wjet"][self.lepton_flavor]["pt"],
            wjet_eta_threshold = self.criteria["wjet"][self.lepton_flavor]["eta"],
            WvsQCD = self.criteria["wjet"][self.lepton_flavor]["particleNet_W_Nominal"],
            syst_var=self.syst
        )
        good_wjets = good_wjets_masks["nominal"] 
        ...
        if self.syst and self.is_mc:
            objects_variations = {
                ...
                "lightjet": good_lightjets_masks,
                "jet": good_jets_masks,
                "topjet": good_topjets_masks,
                "wjet": good_wjets_masks,
                "electron": good_electrons_masks,
                "muon": good_muons_masks,
                "tau": good_taus_masks,
                "met": good_met_masks,
                "delta_phi_jet_met": delta_phi_jet_met_masks
            }
        ...
        # -------------------------------
        #        Trigger: OR (Numerator)
        # -------------------------------      
        trigger_eff = self.criteria["trigger"][self.lepton_flavor]["efficiency"]
        
        if trigger_eff is not None:
            trigger_names_eff, trigger_mask_eff = get_trigger_mask(
                events=events,
                lepton_flavor = self.lepton_flavor,
                year = self.year,
                trigger_case = trigger_eff,
                Or_HLT = self.criteria["trigger"][self.lepton_flavor]["OR_trigger_num"]
            )
            
            self.selections.add("trigger_eff", trigger_mask_eff)

            output["metadata"].update({"Triggers_eff": trigger_names_eff})
        ...
        if self.is_mc:
            ...
            add_pujetid_weight(
                ...
                jet_mask = get_mask_until_object(selections=self.selections, cuts=region_selection[self.lepton_flavor], obj_name="top_tagger" if has_top and not has_bjet else "bjet", include_cut=False, only_cut=True)
            )
            ...
            btag_corrector = BTagCorrector(
                ...
                bjet_mask = get_mask_until_object(selections=self.selections, cuts=region_selection[self.lepton_flavor], obj_name="top_tagger" if has_top and not has_bjet else "bjet", include_cut=False, only_cut=True),
                mask_eff_btag = get_mask_until_object(selections=self.selections, cuts=region_selection[self.lepton_flavor], obj_name="top_tagger" if has_top and not has_bjet else "bjet", include_cut=False, only_cut=False)
            )            
            ...
            add_QCD_vs_Top_weight(
                topjets=objects["topjets"],
                weights=weights_container,
                year=self.year,
                working_point_topjet = self.criteria["topjet"][self.lepton_flavor]["particleNet_Top_Nominal"],
                top_mask = (
                    (objects["events"].top_tagger_case_id == 5) 
                    | (objects["events"].top_tagger_case_id == 6) 
                    | (objects["events"].top_tagger_case_id == 7) 
                    | (objects["events"].top_tagger_case_id == 8) 
                    | (objects["events"].top_tagger_case_id == 13)
                )
            )
            ...
            add_QCD_vs_W_weight(
                ...
                W_mask = (
                    (objects["events"].top_tagger_case_id == 3) 
                    | (objects["events"].top_tagger_case_id == 4) 
                    | (objects["events"].top_tagger_case_id == 11) 
                    | (objects["events"].top_tagger_case_id == 12)
                )
            )
            ...
            #        Trigger weights
            # ************************************
            if self.criteria["trigger"][self.lepton_flavor]["main"] == "tau":
                add_met_trigger_corrections(
                    ...
                    weights_container, 
                    self.year,
                    trigger_mask = get_mask_until_object(selections=self.selections, cuts=region_selection[self.lepton_flavor], obj_name="trigger", include_cut=False, only_cut=True)
                ) 

            
            elif self.criteria["trigger"][self.lepton_flavor]["main"] == "mu":
        ...
        if not self.unblinded and not self.is_mc:
            weights = ak.zeros_like(weights_container.weight())
        else:
            weights = weights_container.weight()

        fill_cutflow(region_selection[self.lepton_flavor], self.selections, "cutflow", output["metadata"]["main"], weights)

        # Delete data if self.unblinded == True (only in SR)
        if ak.sum(weights) == 0:
            for key in output["metadata"]["main"]["cutflow_raw"]:
                output["metadata"]["main"]["cutflow_raw"][key] = 0.0
            
        # =============================================================
        #             Histograms
        # =============================================================
        hist = Histograms(self.lepton_flavor, self.processor, objects, weights, self.selections, region_selection[self.lepton_flavor])
        output["hist"]["main"] = {}
        output["hist"]["main"]["nominal"] = hist.fill_histograms()
        output["hist"]["main"]["nominal"]["sumw_all_weights"] = ak.sum(weights)
        ...
        if self.qcd_data_driven:
            qcd_BCD = QCD_data_driven(
                ...
                output_hist=output["hist"],
                systematic_variations=self.syst,
                lepton_flavor=self.lepton_flavor,
                output_metadata=output["metadata"],
                weights_container=weights_container,
                region_selection_cuts=region_selection[self.lepton_flavor],
                variations=objects_variations if (self.syst and self.is_mc) else None
            )
