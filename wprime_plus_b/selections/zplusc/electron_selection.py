...
def select_good_electrons(
    ...
    electron_eta_mask = (np.abs(events.Electron.eta) < electron_eta_threshold) & (
    ...
    tmp_mask = electron_pt_mask & electron_eta_mask & electron_id_iso_mask

    # select leading electron
    good_electrons = events.Electron.mask[tmp_mask]
    leading_electron = ak.firsts(good_electrons)
    leading_electron_mask = leading_electron.pt > electron_pt_leading


    good_electron_masks = (
        tmp_mask & ak.fill_none(leading_electron_mask, False)
    )


    return good_electron_masks