...
from wprime_plus_b.object_identification.cjet_selection import select_good_cjets
...
class ZplusCProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        good_cjets_masks = select_good_cjets(
            jets=jets_veto,
            year=self.year,
            ctag_working_point = self.criteria["cjet"][self.lepton_flavor]["ctag_wp_pass"],
            jet_pt_threshold = self.criteria["cjet"][self.lepton_flavor]["pt"],
            jet_eta_threshold = self.criteria["cjet"][self.lepton_flavor]["eta"],
            jet_id_wp = self.criteria["cjet"][self.lepton_flavor]["jet_id"],
            jet_pileup_id = self.criteria["cjet"][self.lepton_flavor]["pileup_id"],
            is_mc=self.is_mc,      
        )
        ...
        good_Z_mask = select_good_Z(
                ...
                cross_cleaning = cc,
                year=self.year,
                charge_selection = self.criteria['Z_boson'][self.lepton_flavor]['charge'],
                Z_mass_min = self.criteria['Z_boson'][self.lepton_flavor]['m_Z_min'],
                Z_mass_max = self.criteria['Z_boson'][self.lepton_flavor]['m_Z_max']
        )
        ...
        # ========================================================
        # Dictionary of physics objects (collections)
        # ========================================================
        objects = {
            "bjets": bjets,
            "cjets": cjets,
