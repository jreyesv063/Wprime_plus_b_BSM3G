...
class ZplusCProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        good_cjets_masks = select_good_cjets(
            events=events,
            year=self.year,
            ctag_working_point_pass = self.criteria["cjet"][self.lepton_flavor]["ctag_wp_pass"],
            cjet_pt_threshold = self.criteria["cjet"][self.lepton_flavor]["pt"],
            cjet_eta_threshold = self.criteria["cjet"][self.lepton_flavor]["eta"],
            cjet_id_wp = self.criteria["cjet"][self.lepton_flavor]["jet_id"],
            cjet_pileup_id = self.criteria["cjet"][self.lepton_flavor]["pileup_id"],
            syst_var=self.syst
        )
        good_cjets = good_cjets_masks["nominal"]
        ...
        self.selections.add("one_cjet", ak.num(objects["cjets"]) == 1)
        self.selections.add("cjet_veto", ak.num(objects["cjets"]) == 0)
