...
def add_TopPtReweighting(
    ...
    # ============================================================
    # Correction name
    with open("wprime_plus_b/corrections/correction_names/TopPtReweighting.json", "r") as f:
    ...
    variations = {
        "nominal": (parameters["a"]["nominal"], parameters["b"]["nominal"]),
        "up": (parameters["a"]["up"], parameters["b"]["up"]),
        "down": (parameters["a"]["down"], parameters["b"]["down"]),
    }

    # =============================================================
    # Correction: event-level weight (nominal/up/down)
    # =============================================================  
    # weight = \sqrt{Sf(top) * SF(antitop)}
    nominal_sf, up_sf, down_sf = [
        ak.flatten(np.sqrt(sf(variations[v][0], variations[v][1], top.pt) * sf(variations[v][0], variations[v][1], antitop.pt)))
        for v in ("nominal", "up", "down")
    ]
    
    # --- Add to weights container ---
    weights_container.add(
        f"top_pt_reweighting_{year}",
        weight=nominal_sf,
        weightUp=up_sf,
        weightDown=down_sf,
    )