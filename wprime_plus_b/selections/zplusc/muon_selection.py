...
def select_good_muons(
    ...
    for variation, muon_pt in pt_shifts.items():
        ...
        good_muon_mask = muon_pt_mask & muon_eta_mask & muon_id_mask & muon_iso_mask
        good_muons = events.Muon.mask[good_muon_mask]


        # select leading muon
        leading_muon = ak.firsts(good_muons)
        leading_muon_mask = leading_muon.pt > muon_pt_leading
        

        good_muon_masks[variation] = (
           good_muon_mask & ak.fill_none(leading_muon_mask, False)
        )
