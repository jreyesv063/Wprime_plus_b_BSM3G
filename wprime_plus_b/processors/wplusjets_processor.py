...
from wprime_plus_b.processors.utils.analysis_utils import delta_r_mask, normalize, output_metadata, fill_cutflow 
...
class WplusJetsProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        if nevents_after == 0:
            ...
            fill_cutflow(metadata = output["metadata"], cut_name = "fail_top_tagger", table_name = "cutflow", weights = region_selection_weights[mask_top])
        ...
        if self.output_type == "array":
            array_dict = {}
            self.add_feature("weights_main", region_selection_weights[mask_top])
