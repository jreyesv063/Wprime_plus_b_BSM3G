...
# Selections: Config
from wprime_plus_b.selections.ztoll.cjet_config import ztoll_cjet_selection
...
# Selections: objects
from wprime_plus_b.selections.ztoll.cjet_selection import select_good_cjets
...
class ZToLLProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        good_cjets_masks = select_good_cjets(
            jets=jets_veto,
            year=self.year,
            ctag_working_point=ztoll_cjet_selection[self.channel][
                self.lepton_flavor
            ]["ctag_working_point"],
            jet_pt_threshold=ztoll_cjet_selection[self.channel][
                self.lepton_flavor
            ]["cjet_pt_threshold"],
            jet_eta_threshold =ztoll_cjet_selection[self.channel][
                self.lepton_flavor
            ]["cjet_eta_threshold"],
            jet_id_wp=ztoll_cjet_selection[self.channel][
                self.lepton_flavor
            ]["cjet_id_wp"],
            jet_pileup_id=ztoll_cjet_selection[self.channel][
                self.lepton_flavor
            ]["cjet_pileup_id"],
            is_mc=self.is_mc,      
        )
        ...
        if self.is_mc:
            ...
            cjet_corrector = CTagCorrector(
                jets=events.Jet,
                weights=weights_container,
                working_point=ztoll_cjet_selection[self.channel][self.lepton_flavor][
                    "ctag_working_point"
                ],
                tagger="deepJet",
                year=self.year,
