...
from wprime_plus_b.selections.wplusjets.delta_phi_jet_met_selection import select_good_delta_phi_jet_met
    ...
    def process(self, events):
        ...
        for syst_var in syst_variations:
           ...
           # -------------------------
            ...
            if self.lepton_flavor == "mu":
                mu_id = wjets_muon_selection[self.channel][self.lepton_flavor]["muon_id_wp"]
                with importlib.resources.path(
                    "wprime_plus_b.data", "triggers.json"
                ) as path:
                    with open(path, "r") as handle:
            ...
            # deltaphi_cut cut: 
            # --------------------------     
            delta_phi_cut =  wjets_met_selection[self.channel][self.lepton_flavor]["delta_phi_jet_met"]
            invert_delta_phi =  wjets_met_selection[self.channel][self.lepton_flavor]["invert_delta_phi"]
           ...
           # --------------------------
            ...
            if self.run_systematics and self.is_mc:
               # Taus, muons, bjets, light_jets, fatjets, wjets change due to object-corrections. Electrons don't have object-corrections.
                ...
                region_selection_cases = update_region_map(region_selection[self.channel][self.lepton_flavor], map_variation, met_threshold)
                ...
                for region_name, region_mask in region_selection_map.items():
                    ...
                    if nevents_after == 0:
                        if region_name == "nominal":
                            ...
                            output["metadata"][f"cutflow_{region_name}"]["failing_top_tagger"] = ak.sum(weights_container.weight()[region_mask])
                       ...
                       # ------------------------------------
                        ...
                        if region_name == "nominal":
                            ...
                            output["metadata"][f"cutflow_{region_name}"]["failing_top_tagger"] = ak.sum(weights_container.weight()[region_mask][mask_top])
