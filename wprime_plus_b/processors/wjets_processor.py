...
from wprime_plus_b.corrections.met import apply_met_unclustered, apply_met_phi_corrections, met_recoil
...
class WjetsProccessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        leading_jet_masks = get_leading_jet_mask(
            events=events, 
            jets=objects["jets"], 
            leading_pt_threshold=self.criteria["jet"][self.lepton_flavor]["leading_pt"],
            syst_var = self.syst
        )
        ...
        #     Leading Jet
        # -------------------------    
        self.selections.add("leading_jet", leading_jet_masks["nominal"])
        ...
        self.selections.add("one_jet", ak.num(objects["jets"]) == 1)
        self.selections.add("at_least_one_jet", ak.num(objects["jets"]) >= 1)
        ...
        if self.is_mc:
            ...
            #        Trigger weights
            # ************************************
            if self.criteria["trigger"][self.lepton_flavor]["main"] == "mu":
